...
from src.geometry.shape.capsule import Capsule
from src.utils.math_utils import MathUtils
from robot.robot_config import RobotConfig
from robot.robot import Robot, get_transformation_mdh, wrap
...
class UR5e(Robot):
    def __init__(self) -> None:
        ...
        for i in range(6):
            links.append(rtb.DHLink(d=d_array[i], alpha=alpha_array[i], a=a_array[i], offset=theta_array[i], mdh=True,
                                    m=ms[i], r=rs[i], I=Is[i], Jm=0.1, G=1.0))
    ...
    def ikine(self, Tep: SE3) -> list:
        ...
        # solve theta5
        theta5_condition = -Tep.a[0] * np.sin(thetas[0]) + Tep.a[1] * np.cos(thetas[0])
        ...
        # solve theta6
        m1 = - Tep.n[0] * np.sin(thetas[0]) + Tep.n[1] * np.cos(thetas[0])
        n1 = - Tep.o[0] * np.sin(thetas[0]) + Tep.o[1] * np.cos(thetas[0])
        ...
        qs = [wrap(thetas[i] - self.theta_array[i])[0] for i in range(self.dof)]
